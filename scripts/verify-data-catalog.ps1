...
try {
  ...
  Invoke-CheckedStep "Output removal contracts" { node tests/viewer-contracts.js output-removal }
  ...
  Write-Host "  2. Verify only Template and Data tabs are present; Output controls are absent."
  Write-Host "  3. Open Data; verify Global and Subroutine / FORM groups."
  Write-Host "  4. Verify a nested FORM parameter shows local <- intermediate <- root."
  Write-Host "  5. Edit a traced row, open Template, and verify the same description."
  Write-Host "  6. Switch PERFORM source; verify chain-specific overrides do not bleed."
  Write-Host ""
