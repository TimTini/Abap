...
- `DELETE TABLE itab FROM wa` now captures the table operand after `TABLE`; a parser regression and a Viewer `PERFORM ... CHANGING` trace regression protect the `FA ← A` description chain.
- `SORT ... USING KEY` is not modeled as a valid addition because SAP's current SORT syntax has no `USING KEY` form; the regression keeps it out of structured operands.
...
| Internal tables | Targeted probes now cover `READ TABLE ... WHERE`, secondary `USING KEY`, grouped `LOOP AT`, `LOOP AT GROUP`, and `DELETE TABLE` target binding. Deletion range `TO`/`STEP` and `INSERT`/`MODIFY`/`READ` result additions such as `CASTING` and `ELSE UNASSIGN` remain incomplete. Table expressions are retained as expression text but their complete key/index alternatives are not modeled. Dynamic `SORT BY (otab)` and some direction/text forms are not modeled as structured operands. `SORT ... USING KEY` is excluded because SAP documents no such SORT addition. | [READ TABLE](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPREAD_TABLE.html), [LOOP AT itab](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPLOOP_AT_ITAB.html), [DELETE itab](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPDELETE_ITAB.html), [INSERT itab](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPINSERT_ITAB.html), [MODIFY itab](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPMODIFY_ITAB.html), [SORT itab](https://help.sap.com/doc/abapdocu_latest_index_htm/latest/en-US/ABAPSORT_ITAB.html) |
