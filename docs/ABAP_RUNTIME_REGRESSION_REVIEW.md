...
- Fixed the source in `shared/abap-parser.js`: comments on declaration/signature objects now populate `codeDesc` only for the declared `name` entry. A 12-kind parser matrix covers DATA, CLASS-DATA, CONSTANTS, FIELD-SYMBOLS, FORM, PARAMETERS, RANGES, SELECT-OPTIONS, STATICS, TYPES, METHODS, and CLASS-METHODS.
...
- Final parser verification: `node tests/run.js parser` passed, including the 39 statement regressions, 9 condition regressions, SAP-latest coverage, grammar inventory, detailed API, and parser contracts.
- Final Viewer/runtime verification: `node tests/run.js fast` passed after rebuilding the inline artifact. Its Viewer contracts include the new exact spaced/lowercase chained DATA case and nested PERFORM description trace. The first fast run correctly failed because `viewer/index.inline.html` was stale; after `uv run python scripts/build-inline-viewer.py`, the rerun passed.
- Release verification: `node tests/run.js full` passed after the final source and generated-artifact changes.
...
- The reviewed implementation and generated Viewer artifacts are ready for the authorized commit/push. Keep the untracked user-owned `.serena/` and `references/` directories out of Git staging.
