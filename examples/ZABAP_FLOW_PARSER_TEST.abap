REPORT zabap_flow_demo.

* Sources (declare everything you pass as parameters)
DATA gv_user  TYPE syuname   VALUE sy-uname.    "User name (source for iv_user)
DATA gv_in    TYPE string    VALUE '123'.       "Input string (source for iv_in)
DATA gv_demo  TYPE abap_bool VALUE abap_true.   "Run demo_all (source for iv_demo)
DATA gv_cycle TYPE abap_bool VALUE abap_true.   "Run mutual recursion (source for iv_cycle)
DATA gv_self  TYPE abap_bool VALUE abap_true.   "Run self recursion (source for iv_self)
DATA gv_maxd  TYPE i         VALUE 3.           "Max recursion depth (source for iv_maxd)

CONSTANTS gc_depth_start TYPE i VALUE 1.        "Initial depth for recursion calls

DATA gv_count TYPE i.                           "Global counter (CHANGING cv_count)
DATA gv_text  TYPE string.                      "Global output text (CHANGING cv_text)
DATA gt_log   TYPE TABLE OF string.             "Global log table (TABLES pt_log)

START-OF-SELECTION.
  PERFORM main USING gv_user gv_in gv_demo gv_cycle gv_self gv_maxd
...
FORM main
  ...
  DATA lv_a TYPE i VALUE 0.        "Input number (passed to step_a/step_b)
  DATA lv_b TYPE string.           "Output buffer (CHANGING in step_b/pass_changing)
  ...
  * Độ sâu
  ...
  IF iv_cycle = abap_true.
    PERFORM step_cycle_1 USING gc_depth_start iv_maxd.
  ENDIF.

  IF iv_self = abap_true.
    PERFORM self_call USING gc_depth_start iv_maxd.
  ENDIF.
...
FORM step_c
  ...
  cv_count = cv_count + 1.
  PERFORM step_cycle_2 USING gc_depth_start iv_maxd.
