(function (ns) {
  ...
  function hasVisibleCellText(cellCfg) {
    const t = cellCfg && typeof cellCfg === "object" ? String(cellCfg.text ?? "") : "";
    return Boolean(t.trim());
  }

  function hasCellBlockingContent(cellCfg) {
    if (!cellCfg || typeof cellCfg !== "object") return false;
    const t = String(cellCfg.text ?? "").trim();
    if (t) return true;
    const bind = String(cellCfg.bind ?? "").trim();
    return Boolean(bind);
  }

  function nextBlockingCol(blockColsSorted, col) {
    if (!Array.isArray(blockColsSorted) || !blockColsSorted.length) return 0;
    for (let i = 0; i < blockColsSorted.length; i++) {
      const c = Number(blockColsSorted[i]);
      if (Number.isFinite(c) && c > col) return c;
    }
    return 0;
  }
  ...
  function renderExcelLikeTable(config) {
    ...
    const excelOverflow = Boolean(config?.grid?.excelOverflow);
    ...
    for (const c of cells) {
      ...
      if (excelOverflow) {
        ...
        if (!parsed) continue;
        if (!hasCellBlockingContent(c)) continue;
        if (!rowBlocking.has(parsed.row)) rowBlocking.set(parsed.row, []);
        rowBlocking.get(parsed.row).push(parsed.col);
      }
    }
    if (excelOverflow) {
      for (const [r, list] of rowBlocking.entries()) {
        const sorted = list
          .map((x) => Number(x))
    ...
    for (let c = 1; c <= cols; c++) {
      ...
      colPrefix[c] = totalWidth;
    }
    table.appendChild(colgroup);
    if (totalWidth > 0) table.style.width = `${totalWidth}px`;

    function spanWidthPx(fromCol, toCol) {
      const from = Math.max(1, Math.floor(Number(fromCol || 1)));
      const to = Math.max(1, Math.floor(Number(toCol || 1)));
      if (to < from) return 0;
      const start = colPrefix[from - 1] || 0;
      const end = colPrefix[to] || 0;
    ...
    for (let r = 1; r <= rows; r++) {
      ...
      for (let c = 1; c <= cols; c++) {
        ...
        if (excelOverflow) {
          ...
          td.style.zIndex = String(Math.max(1, cols - c + 1));
        }

        if (cellCfg) {
          const text = String(cellCfg.text ?? "");
          const allowOverflow = excelOverflow && hasVisibleCellText(cellCfg);

          if (allowOverflow) {
            td.style.overflow = "visible";

            const nextBlock = nextBlockingCol(rowBlocking.get(r), c);
            const limitCol = nextBlock ? Math.max(c, nextBlock - 1) : cols;
            const maxWidth = spanWidthPx(c, limitCol);
