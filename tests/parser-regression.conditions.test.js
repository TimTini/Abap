...
defineFocusedTest(test, "parser condition regression", ["conditions"], async (t) => {
  await t.test("read table with key conditions", () => {
    testReadTableWithKeyConditions();
  });

  await t.test("read table with table key conditions", () => {
    testReadTableWithTableKeyConditions();
  });

  await t.test("where condition extras for similar statements", () => {
    testWhereConditionExtrasForSimilarStatements();
  });

  await t.test("condition extras for if elseif select and perform", () => {
    testConditionExtrasForIfElseifSelectAndPerform();
  });

  await t.test("condition operator matrix", () => {
    testConditionOperatorMatrix();
  });

  await t.test("is initial and is not initial conditions", () => {
    testIsInitialAndIsNotInitialConditions();
  });

  await t.test("implicit split only for read table", () => {
    testImplicitSplitOnlyForReadTable();
  });

  await t.test("condition operands always have decl", () => {
