...
window.AbapViewerModules.parts = window.AbapViewerModules.parts || {};
  ...
  function walkFilteredTree(nodeInfo, visit) {
    ...
    for (const child of children) {
      walkFilteredTree(child, visit);
    }
  }

  function buildOutputVirtualMaps(filteredRoots) {
    const idToRootIndex = new Map();
    const lineTargetMap = new Map();

    for (let rootIndex = 0; rootIndex < filteredRoots.length; rootIndex += 1) {
      const rootInfo = filteredRoots[rootIndex];
      walkFilteredTree(rootInfo, (nodeInfo) => {
        const obj = nodeInfo && nodeInfo.obj ? nodeInfo.obj : null;
        if (!obj) {
          return;
        }
        const id = normalizeId(obj.id);
        if (id && !idToRootIndex.has(id)) {
          idToRootIndex.set(id, rootIndex);
        }
  ...
  function buildOutputRootElement(rootInfo, rootIndex) {
    const node = renderNode(rootInfo);
    node.setAttribute("data-virtual-root-index", String(rootIndex));
    return node;
  }

  function applyOutputScrollDelta(delta) {
    if (!els.output || Math.abs(Number(delta) || 0) < 1) {
      return;
    }
    const virtual = getOutputVirtualState();
    virtual.isAdjustingScroll = true;
    const nextTop = Math.max(0, (Number(els.output.scrollTop) || 0) + Number(delta || 0));
    els.output.scrollTop = nextTop;
  ...
  function renderOutputVirtualInitialRange(options) {
    ...
    const opts = options && typeof options === "object" ? options : {};
    const virtual = getOutputVirtualState();
    const roots = Array.isArray(virtual.roots) ? virtual.roots : [];
    ...
    for (let index = start; index < end; index += 1) {
      const elNode = buildOutputRootElement(roots[index], index);
  ...
  function initOutputVirtualWindow(filteredRoots, options) {
    ...
    const maps = buildOutputVirtualMaps(roots);
    virtual.roots = roots;
    virtual.itemCount = roots.length;
    virtual.idToRootIndex = maps.idToRootIndex;
    virtual.lineTargetMap = maps.lineTargetMap;
    virtual.start = 0;
    virtual.end = 0;
    virtual.scrollDir = "down";
    virtual.isAdjustingScroll = false;
    virtual.isInitialized = roots.length > 0;
    ...
    renderOutputVirtualInitialRange({
      preserveScroll: selectedRootIndex < 0 && opts.preserveScroll === true,
  ...
  function ensureOutputWindowContainsRootIndex(rootIndex) {
    ...
    virtual.start = start;
    virtual.end = end;
    renderOutputVirtualInitialRange({ preserveScroll: false, scrollTop: 0 });
    return true;
  }

  function appendOutputBatch(batchCount) {
    if (!els.output) {
      return 0;
    }
    const virtual = getOutputVirtualState();
    ...
    for (let index = oldEnd; index < newEnd; index += 1) {
      const node = buildOutputRootElement(virtual.roots[index], index);
      els.output.appendChild(node);
      heights.push(measureOuterHeight(node));
    }
    virtual.end = newEnd;
    updateAverageItemHeight(virtual, heights, 80);
  ...
  function prependOutputBatch(batchCount) {
    if (!els.output) {
      return 0;
    }
    const virtual = getOutputVirtualState();
    ...
    updateAverageItemHeight(virtual, heights, 80);
    applyOutputScrollDelta(addedHeight);
    return nodes.length;
  }

  function dropOutputTop(dropCount) {
    if (!els.output) {
      return 0;
    }
    const virtual = getOutputVirtualState();
    ...
    let removedHeight = 0;
    while (removed < count) {
      const first = els.output.firstElementChild;
      if (!first) {
        break;
      }
      removedHeight += measureOuterHeight(first);
      first.remove();
      removed += 1;
    }
    ...
    applyOutputScrollDelta(-removedHeight);
    return removed;
  }

  function dropOutputBottom(dropCount) {
    if (!els.output) {
      return 0;
    }
    const virtual = getOutputVirtualState();
    ...
    let removed = 0;
    while (removed < count) {
      const last = els.output.lastElementChild;
      if (!last) {
        break;
      }
      last.remove();
  ...
  function processOutputVirtualScrollFrame() {
    ...
    if (nearTop && virtual.start > 0) {
      prependOutputBatch(metrics.batchCount);
    }
    const rangeCount = virtual.end - virtual.start;
    if (rangeCount > metrics.maxCount) {
      const drop = Math.min(metrics.batchCount, rangeCount - metrics.targetCount);
      dropOutputBottom(drop);
    }
  }
